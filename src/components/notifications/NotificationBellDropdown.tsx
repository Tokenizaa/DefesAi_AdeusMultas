  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-extrabold text-white animate-pulse">
        ...
        <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl bg-[#071D41] border border-[#155BCB]/40 shadow-2xl shadow-black/80 z-50 overflow-hidden text-white animate-in fade-in slide-in-from-top-2 duration-150">
          ...
          <div className="px-4 py-3 bg-[#0C326F]/60 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-white">Notificações do Sistema</h3>
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="text-[11px] text-slate-400 hover:text-[#FFCD07] flex items-center gap-1 transition-colors"
              >
          ...
          {permission !== 'granted' && (
            <div className="p-3 bg-gradient-to-r from-[#155BCB]/30 to-[#071D41] border-b border-[#155BCB]/40 flex items-center justify-between gap-2">
              <div className="text-xs text-slate-300">
                ...
                <span className="text-[11px] text-slate-400">Receba notificações de julgamento no mobile.</span>
              </div>
              <button
                onClick={requestPermissionAndSubscribe}
                disabled={loading}
                className="bg-[#FFCD07] hover:bg-[#F5A623] text-[#071D41] font-bold text-[11px] px-3 py-1.5 rounded-lg shrink-0 transition-colors shadow-sm"
              >
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-white/5">
            {(!notifications || notifications.length === 0) ? (
              <div className="py-8 px-4 text-center">
                ...
                <p className="text-xs text-slate-400">Nenhuma notificação registrada no momento.</p>
              </div>
            ) : (
              (notifications || []).map((notif) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-300 line-clamp-2 leading-relaxed">{notif.body}</p>
                  </div>
                </div>
              ))
            )}
          ...
          {/* Footer Actions */}
          <div className="p-2.5 bg-[#0C326F]/40 border-t border-white/10 flex items-center justify-between text-xs">
            <button
              onClick={sendTestNotification}
              disabled={loading}
              className="text-[11px] text-slate-300 hover:text-white flex items-center gap-1.5 px-2 py-1 rounded hover:bg-white/10 transition-colors"
            >
            ...
            <button
              ...
              className="text-[11px] text-[#FFCD07] hover:underline flex items-center gap-1 font-semibold"
            >
