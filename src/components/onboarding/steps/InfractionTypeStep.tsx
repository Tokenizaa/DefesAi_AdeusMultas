  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-lg space-y-6">
      <div className="text-center max-w-xl mx-auto space-y-4">
        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold uppercase tracking-wider bg-orange-50 text-orange-700 border border-orange-200 font-mono">
          <Sparkles className="w-4 h-4 text-orange-500" />
          Passo 3 de 10 • Qual foi o motivo da autuação?
        </span>
          ...
          >
            <div className="flex w-full items-start gap-4 mb-3">
              <div className="flex-shrink-0">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedType === option.value ? 'bg-orange-50' : 'bg-slate-100 text-slate-700'}`}>
                  {option.icon}
                </div>
              </div>
              <div className="flex-1 space-y-2">
                <h3 className="font-semibold text-slate-900">{option.title}</h3>
                <p className="text-base text-sm">{option.description}</p>
              </div>
            </div>
          </button>
        ))}
