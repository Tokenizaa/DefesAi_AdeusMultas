...
export const GoogleDriveButton: React.FC<GoogleDriveButtonProps> = ({
  ...
  const getButtonStyles = () => {
    switch (variant) {
      ...
      case 'compact':
        return 'p-2 bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-200 text-xs';
      case 'outline':
      default:
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex items-center justify-center gap-2 px-3 py-2 border rounded-xl text-xs font-semibold shadow-2xs transition-all cursor-pointer ${getButtonStyles()} ${className}`}
